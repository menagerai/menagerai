...
- **A ForwardAuth gateway** (Traefik today; more gateways over time) that authenticates and
...
- **Then** — broader provider support (more gateways, identity providers, and database
  options over time) behind the existing pluggable abstractions. Storage runs on a
  bundled **SQLite** file by default, with **MongoDB** as a pluggable alternative today.
