...
One zero-dependency `node:http` server serving one static HTML page + logo/favicon
+ a `/healthz` probe. **No configuration** — the two links (the live demo and the
GitHub repo) are hardcoded in `index.html`; edit them there if you fork.
...
Set the domain to your root + `www.` (e.g. `menager.ai,www.menager.ai`). No env
vars. The demo itself is a separate resource (see [`../README.md`](../README.md)) on
`demo.menager.ai` — the demo link on the page points there.
